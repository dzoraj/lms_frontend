...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, NavbarComponent, MatTabsModule, MatCardModule, MatProgressBarModule,RouterLink],
      ...
      next: (dashboard) => {
        ...
        if (dashboard?.passedExams?.length) {
          dashboard.passedExams = dashboard.passedExams.map((pe: any) => {
            ...
            if ((!pts || pts === 0) && subjectInfo) {
              pts = subjectInfo.total;
            }
